...
const AllMyPetitions = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5001/slpp/petitions')
      ...
      .then((data) => {
        ...
        const filedPetitions = allPetitions.filter((petition) => petition.petitioner === userEmail);
        setPetitions(filedPetitions);
        setLoading(false);
      })
      .catch((error) => {
  ...
  if (petitions.length === 0) {
    return <div className="text-center text-lg text-gray-500">You haven't filed any petitions yet.</div>;
  }
